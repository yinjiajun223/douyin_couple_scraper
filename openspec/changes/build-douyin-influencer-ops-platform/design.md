...
- Give every screening run a reproducible rule snapshot, idempotent ingestion, traceable AI output, and auditable human decisions.
...
- Isolate platform-specific and AI-provider-specific code so later adapters do not force a rewrite of the core domain.
...
- Treating inferred age, amateur status, or content fit as verified facts or automatic rejection grounds.
...
- `apps/worker`: a separate Node.js process importing the same domain services for durable background jobs.
...
- AI rules: likely age range 18–24 and likely amateur/ordinary creator;
- optional AI or manual content-fit prompts, with no default content-category restriction;
- run safety settings: maximum feed items, maximum creator profiles, pacing range, and maximum duration.

The rule engine returns `pass`, `fail`, or `unknown` per rule plus machine-readable evidence. Missing or unparseable values produce `unknown`, never zero. Hard rules run before AI scheduling; AI and manual rules may rank or annotate a candidate but cannot silently delete it. Evaluation records point to the exact rule version and observation IDs used.
...
- decisions: `campaign_candidates`, `rule_evaluations`, `ai_analysis_runs`, `manual_reviews`, `outreach_records`, and `candidate_events`;
- infrastructure: `media_objects`, `background_jobs`, and `ingestion_keys`.
...
### 5. Use MySQL-backed durable jobs instead of introducing Redis

AI requests, retries, media cleanup, exports, and maintenance tasks are represented in a MySQL job table. Workers claim small batches using row locks with `SKIP LOCKED`, record lease ownership and expiry, and apply exponential retry with a dead-letter state. Jobs use deterministic deduplication keys where repeated execution would be harmful.

For the expected first-version team size, this is sufficient and avoids adding Redis to a small server. Worker concurrency defaults to one AI job and a small number of lightweight maintenance jobs, and can be raised through configuration after observing CPU, memory, provider limits, and RDS load.

A memory-only queue was rejected because deployments and crashes would lose work. Redis-backed queues remain a future option if job throughput or scheduling sophistication outgrows MySQL.

### 6. Upload evidence directly to private OSS
...
### 7. Treat AI as an optional, provider-agnostic enrichment stage

An AI connection contains a label, adapter type, base URL, model, capability flags, limits, and an encrypted credential. The initial adapter supports an OpenAI-compatible chat/multimodal protocol, but all calls pass through an internal provider interface. A connection test verifies authentication, text input, image input, and schema-constrained JSON before the connection can be enabled for production tasks.

Credentials are encrypted at application level with a deployment master key held outside the database. Responses and logs must redact authorization headers and secret values. The worker sends only the configured minimum evidence: selected public screenshots, profile text, and captions. It does not send cookies, contact notes, private team remarks, or full videos.

Every analysis stores provider/model identifiers, prompt/schema version, input evidence references, timestamps, outcome, and raw-response metadata needed for debugging. The normalized result includes age-band estimate, amateur-status estimate, content tags, suitability signals, risk flags, evidence statements, and per-field confidence. Invalid output is retried within limits and otherwise marked unavailable; candidates remain reviewable.

Binding directly to one vendor SDK was rejected because the available company endpoint may be wrapped and other vendors must remain usable. Allowing AI output to mutate final review status was rejected because age and identity-style inference are uncertain and require accountable human judgment.

### 8. Use invite-only web sessions and scoped device credentials
...
### 9. Isolate Douyin behavior behind a confidence-aware platform adapter
...
### 10. Deploy a small-container topology on the existing Ubuntu server
...
Health endpoints distinguish process liveness from readiness of MySQL and OSS. Structured logs include request, run, device, job, and candidate correlation IDs without secrets. Database migrations run as a controlled release step, not automatically from every replica. RDS backups and OSS versioning/lifecycle policy are complemented by a documented restore drill.
...
### 11. Protect concurrent human work with optimistic versioning and event history
...
### 13. Preview private evidence inside an accessible zoomable lightbox
...
- [AI age and “amateur” inference is inaccurate or biased] → Label all results as estimates, retain visible evidence and confidence, prohibit automatic rejection, and require human review before outreach.
- [External AI leaks data, fails, or becomes expensive] → Minimize inputs, never send cookies or private CRM fields, hard-filter first, cap per-run analyses, encrypt credentials, retain provider provenance, and degrade to manual review.
- [The 2-core/4-GB/1-Mbps host becomes constrained] → Keep browsers off-server, upload directly to OSS, start with low worker concurrency and container limits, and measure queue depth, latency, memory, and disk before scaling.
...
- [A member accesses another operator's creator through a guessed ID or secondary endpoint] → Resolve record scope in shared domain authorization and apply it consistently to list, detail, export, media, AI, review, and outreach paths; cover every path with cross-member tests.
...
6. Enable one AI connection only after its text, image, and structured-output connection tests pass. Run AI on a bounded candidate set and compare results with human review.
7. Pilot with a small operator group, verify administrator-wide and member-scoped record access, then enable invitations, scoped review, outreach tracking, and exports for the wider team.
8. Optionally import existing JSON/CSV results and screenshots as a clearly labeled legacy run; missing fields remain unknown and no AI conclusion is fabricated.
9. Remove the legacy script from the normal workflow only after the central candidate count, evidence links, pause/resume behavior, and retry recovery have passed acceptance checks.
...
- Which external AI endpoint and model will be enabled first after capability testing.
