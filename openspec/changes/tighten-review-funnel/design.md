...
### D1: 闸门只作用于候选记录的「创建」，不作用于「更新」

`findOrCreateCandidate` 改为：命中已存在行 → 照旧更新（含 `hard_filter_status`）；未命中 → 仅 `outcome === 'pass'` 时插入。

**为什么**：全量闸门会让「第一次 pass 进库、运营已在跟进、第二次采集指标掉出区间」的候选停在过时的 `latest_creator_observation_id` 上，且其 `hard_filter_status` 与实际数据脱节。已晋级即持续跟踪，是可解释性更强的语义。

**代价**：库中仍可能出现 `hard_filter_status = 'fail'` 的行。这是正确状态而非 bug，需在 UI 上以「曾通过后掉出区间」的方式呈现，不能与「从未入库的 fail」混淆。

**备选**：全量闸门（更新也跳过）—— 被否，产生过时数据；非 pass 时把候选软归档 —— 被否，会自动归档运营正在跟进的达人，且与 `admin-data-and-member-operations` 的人工归档语义冲突。
...
**[`hard_filter_status` 列语义漂移]** → 新行几乎恒为 pass，历史行仍有三值，且 D1 保留了「曾通过后掉出区间」的 fail 行。保留列与 CHECK 不动，在实现中明确该列现在表示「最近一次运行的硬筛结论」而非「入库资格」。
...
- 孤儿回收宽限期的具体取值（默认建议 7 天，经环境变量可调）—— 不改变 spec 与方案，可在实现时按实际入库延迟确定。
- 「跟进中」与「不合适」两个分区是否要进一步拆分到 7 个 pipeline 值各一个 tab —— spec 只要求「至少五个分区」，属 UI 细节，可在前端实现时按运营反馈调整。
- 是否为管理员提供历史非通过数据的界面入口（当前 spec 只要求服务端接口保留、默认界面不提供）—— 可延后，取决于运营是否真的需要查。
- D2 的重算入口暴露为管理员诊断页面还是仅内部能力 —— 不影响 spec，实现时定。
