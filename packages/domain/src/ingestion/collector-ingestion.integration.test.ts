...
import { createAiConnection } from '../ai/connection-service.js';
import { CredentialCipher } from '../ai/credential-cipher.js';
import { listAiAnalysisHistory, queueAiReanalysis, runAiAnalysis } from '../ai/analysis-service.js';
...
describeWithMysql('Collector 规范化与跨来源去重', () => {
  ...
  it('两个设备在不同任务发现同一账号时共享主档案并保留两条来源', async () => {
    ...
    expect(detail.evaluations).toContainEqual(
      expect.objectContaining({
        ...
        ruleVersion: 1,
        ruleSnapshot: expect.objectContaining({ schemaVersion: 1 }),
  ...
  it('串联默认规则、OSS 证据、AI 建议与人工复核，并保留通过/失败/未知结论', async () => {
    const aiCipher = CredentialCipher.fromSingleKey('s'.repeat(32));
    const aiConnection = await createAiConnection(pool, aiCipher, {
      actorUserId,
    ...
    await pool.execute("UPDATE ai_connections SET status = 'enabled' WHERE id = ?", [
      aiConnection.id,
    ]);
    const configuredRules = createDefaultCampaignRuleSet();
    configuredRules.aiLimits = { budgetCents: 10, concurrency: 2, maximumCandidates: 1 };
    const campaign = await createCampaign(pool, {
      workspaceId,
      actorUserId,
      name: '三态候选测试任务',
    ...
    const [aiJobs] = await pool.query<RowDataPacket[]>(
      `SELECT jobs.payload_json, creators.platform_creator_id
       FROM background_jobs jobs
       JOIN campaign_candidates candidates
         ON candidates.id = JSON_UNQUOTE(JSON_EXTRACT(jobs.payload_json, '$.candidateId'))
       JOIN creators ON creators.id = candidates.creator_id
       WHERE jobs.job_type = 'ai-screening'
         AND JSON_UNQUOTE(JSON_EXTRACT(jobs.payload_json, '$.runId')) = ?`,
      [run.id],
    );
    ...
    const passCandidate = candidates.find(
      (candidate) => candidate.platform_creator_id === 'candidate-pass',
    )!;
    ...
    const loadEvidence = async () => ({
      biography: '校园生活',
      nickname: '三态候选 1',
      posts: [{ caption: '校园爆款记录', sourceId: 'post-observation:pass' }],
      rules: configuredRules,
      selectedScreenshots: [
        {
          sourceId: `media:${mediaUpload.id}`,
          url: mediaAccess.downloadUrl,
        },
      ],
    });
    ...
    const providerFactory = () => ({
      complete: async () => ({
        content: JSON.stringify({
          amateurStatus: { confidence: 0.7, evidence: [], value: 'likely-amateur' },
          contentTags: ['校园'],
          estimatedAge: { confidence: 0, evidence: [], reason: '证据不足', status: 'unknown' },
          riskFlags: [],
          schemaVersion: 1,
          suitability: { confidence: 0.6, evidence: [], value: 'review' },
          summary: `分析版本 ${++analysisSequence}`,
        }),
        finishReason: 'stop',
        model: 'integration-model',
        requestId: `request-${analysisSequence}`,
    ...
    await runAiAnalysis(
      ...
      aiCipher,
      workspaceId,
      aiJobs[0]!.payload_json.analysisId as string,
      loadEvidence,
      providerFactory,
    );
    const reanalysis = await queueAiReanalysis(pool, {
      actorRole: 'admin',
      actorUserId,
      candidateId,
      workspaceId,
    ...
    await runAiAnalysis(
      ...
      aiCipher,
      workspaceId,
      reanalysis.analysisId,
      loadEvidence,
      providerFactory,
    );
    const history = await listAiAnalysisHistory(pool, workspaceId, candidateId);
    expect(history).toHaveLength(2);
    expect(history.map((item) => item.result?.summary)).toEqual(['分析版本 2', '分析版本 1']);
    expect(JSON.stringify(history)).not.toContain('signature=short-lived');
