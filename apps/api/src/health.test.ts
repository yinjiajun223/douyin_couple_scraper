...
describe('service readiness', () => {
  it('distinguishes a required MySQL failure from optional component degradation', async () => {
    ...
    expect(report.components).toEqual({
      optional: { errorType: 'TypeError', required: false, status: 'down' },
      mysql: { errorType: 'MysqlUnavailable', required: true, status: 'down' },
      oss: { required: true, status: 'up' },
    });
  });

  it('reports an OSS outage separately and keeps an optional component outage degraded', async () => {
    const ossFailure = await runReadinessChecks({
      optional: { check: async () => undefined, required: false },
      mysql: { check: async () => undefined, required: true },
      oss: { check: async () => Promise.reject(new Error('timeout')), required: true },
    });
    const optionalFailure = await runReadinessChecks({
      optional: { check: async () => Promise.reject(new Error('rate limited')), required: false },
      mysql: { check: async () => undefined, required: true },
      oss: { check: async () => undefined, required: true },
    });
    ...
    expect(optionalFailure.status).toBe('degraded');
    expect(optionalFailure.components.optional?.status).toBe('down');
  });
});
